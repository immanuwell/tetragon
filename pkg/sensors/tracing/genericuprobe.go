...
type genericUprobe struct {
	...
	tags []string
	// digests field of the Tracing Policy
	digests []string
...
func handleGenericUprobe(r *bytes.Reader) ([]observer.Event, error) {
	...
	unix.Offset = uprobeEntry.offset
	unix.Address = uprobeEntry.address
...
func loadMultiUprobeSensor(ids []idtable.EntryID, args sensors.LoadProbeArgs) error {
	...
	data.Digests = make(map[string][]string)
	...
	for index, id := range ids {
		...
		data.Digests[uprobeEntry.path] = uprobeEntry.digests
	}
...
func addUprobe(spec *v1alpha1.UProbeSpec, ids []idtable.EntryID, in *addUprobeIn, has *uprobeHas) ([]idtable.EntryID, error) {
	...
	addUprobeEntry := func(sym string, offset uint64, addr uint64, addr_type AddressType, idx int) error {
	...
	if symbols != 0 {
		for idx, sym := range spec.Symbols {
			err = addUprobeEntry(sym, 0, 0, Symbol, idx)
			if err != nil {
				return nil, err
			}
		}
		...
		for idx, off := range spec.Offsets {
			err = addUprobeEntry("", off, 0, Offset, idx)
			if err != nil {
				return nil, err
			}
		}
		...
		for idx, addr := range spec.Addrs {
			err = addUprobeEntry("", 0, addr, Address, idx)
			if err != nil {
				return nil, err
			}
		}

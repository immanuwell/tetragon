...
func getAddress(f *os.File, path, configSymbol string, configAddress, configOffset uint64) (string, uint64, uint64, error) {
	...
	elfFile, err := tetragonelf.NewSafeELFFile(file)
	if err != nil {
...
func uprobeAttachSingle(load *Program, prog *ebpf.Program, spec *ebpf.ProgramSpec,
	...
	linkFn := func() (link.Link, error) {
		...
		// a file descriptor directly. But we can use the /proc/self/fd/<N> symlink
...
func attachSingleMultiUprobe(load *Program, prog *ebpf.Program, path string, attach *MultiUprobeAttachSymbolsCookies, digests []string, bpfDir string, idx int, extra ...string) (link.Link, error) {
	// The kernel tracks uprobe targets by inode. When we open a file,
	// its file descriptor points to an inode.
	// The loader API requires that a path is provided. It will not accept
	// a file descriptor directly. But we can use the /proc/self/fd/<N> symlink
...
func uprobeAttachMulti(load *Program, prog *ebpf.Program, spec *ebpf.ProgramSpec,
	...
	linkFn := func() ([]link.Link, error) {
		...
		for path, attach := range data.Attach {
			digests := data.Digests[path]
			lnk, err := attachSingleMultiUprobe(load, prog, path, attach, digests, bpfDir, idx, extra...)
			if err != nil {
				return nil, err
			}
